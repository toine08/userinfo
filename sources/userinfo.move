module sender::user_info {
    //imports Strings type from module string that resides on address std.
    //std is an address defined in the transactive std dependency of the APtosFramework dependency
    //and automoatically available
    ...
    public fun get_username(user_addr: address): String acquires UserProfile{
        // no need for the semicolon at the end, last expression is returning one(just like in rust)
    ...
    public entry fun set_username(user_account: &signer, username_raw: vector<u8>) acquires UserProfile{
        ...
        if(!exists<UserProfile>(user_addr)){
            ...
            //'borrow_global_mute' is to fetch mutable references, we can change resources in storage that way
